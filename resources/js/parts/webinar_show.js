    ...
    $('body').on('click', '.js-play-video', function (e) {
        ...
        $.post('/course/getFilePath', {file_id: file_id}, function (result) {
            if (result && result.code === 200) {
                ...
                let html = '';
                if (storage === 'local') {
                    html = '<video id="my-video" oncontextmenu="return false;" controlsList="nodownload" class="video-js" controls preload="auto" width="870" height="364"><source src="' + result.path + '" type="video/mp4"/></video>';
                } else {
                    html = '<video\n' +
                        '    id="my-video"\n' +
                        '    class="video-js"\n';

                    if (result.storageService !== 'vimeo') {
                        html += '    controls\n';
                    }
